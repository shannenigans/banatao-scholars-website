...
export default function ScholarsDirectoryPage() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const fetched = ((await fetchScholars()) as Scholar[]) ?? [];
        setAllScholars(fetched.filter(isVisible));
      } catch {
        setAllScholars([]);
      } finally {
        setIsLoading(false);
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [debouncedQuery, yearFilters, majorFilters, schoolFilters]);
  ...
  return (
    <div className="container mx-auto px-4 py-12 sm:px-6 lg:px-8">
      ...
      <p className="mt-4 text-sm text-muted-foreground">
        {isLoading ? 'Loading…' : `${filtered.length} ${filtered.length === 1 ? 'scholar' : 'scholars'}`}
      </p>
      ...
      {isLoading ? (
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {paged.map((s) => (
            <ScholarCard key={s.id} scholar={s} showContacts={isAuthed} />
          ))}
        </div>
      )}
